...
def carregar_dados_rebalanceamento(confidence: int, dias_range=range(1, 8)):
    base_path = f"simulations/historic_simulation_il/brl_days_rebalancing/confidence{confidence}"
    dados_por_periodo = []

    for dias in dias_range:
        filename = f"shifted_rebalance{dias}d{confidence}50.csv"
...
def plot_rebalance_costs(df, confidence: int, output_path: str):
    ...
    for dias in sorted(df["rebalance_days"].unique()):
        ...
        plt.plot(dados_dia["date_start"], dados_dia["cumulative_rebalance_cost"], label=f"{dias}d")
    ...
    plt.title(f"BRL Cumulative Rebalance Cost - Confidence {confidence}%")
    plt.xlabel("Date Start")
    plt.ylabel("Cumulative Rebalance Cost (USD)")
...
def main():
    ...
    output_dir = "graphs/brl/rebalance_costs_by_confidence/"
    caminhos_criados = []
    ...
    for confidence in confidence_levels:
        ...
        if df is not None:
            output_file = os.path.join(output_dir, f"rebalance_costs_confidence{confidence}.png")
