"""Fill KTB prices from last ClickHouse daily row until now via eth_getLogs.

Persists after every chunk (and appends each new price to an NDJSON cache) so an
...
DECIMALS = 8
DAILY_PATH = ROOT / "json" / "oracle-ktb" / "ktb_daily_prices.json"
CH_ONLY_PATH = ROOT / "json" / "oracle-ktb" / "ktb_daily_prices_clickhouse_only.json"
OUT_PATH = ROOT / "json" / "oracle-ktb" / "ktb_prices_gap_from_clickhouse.json"
CACHE_NDJSON = ROOT / "json" / "oracle-ktb" / "ktb_prices_gap_cache.ndjson"
CHUNK = 100_000
...
def main() -> None:
    ...
    if state.get("complete") and start > latest:
        print("already complete up to previous latest; checking for new blocks...")
    ...
    while start <= latest:
        ...
        for log in logs:
            ...
            # append immediately so a crash mid-chunk still keeps this price
            append_prices_to_cache([p])

        state["next_block"] = end + 1
        state["to_block"] = latest
        state["elapsed_seconds"] = round(time.time() - t0, 2)
        # full JSON checkpoint after each chunk (resume cursor = next_block)
        save_checkpoint(state)
    ...
    if state["prices"]:
        print(f"first: {state['prices'][0]['date']} {state['prices'][0]['price']}")
        print(f"last:  {state['prices'][-1]['date']} {state['prices'][-1]['price']}")
