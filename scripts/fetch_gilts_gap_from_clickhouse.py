"""Fill GILTS/GBP prices from last ClickHouse daily row until now via eth_getLogs.
...
CONTRACT = "0x3821A301b2cd8eB27831C54De61a8465C1C29b46"
TOPIC0 = "0x0559884fd3a460db3073b7fc896cc77986f16e378210ded43186175bf646fc5f"
DECIMALS = 8
PAIR_NATIVE = "GILTS/GBP"
DAILY_PATH = ROOT / "json" / "oracle-gilts" / "gilts_daily_prices.json"
CH_ONLY_PATH = ROOT / "json" / "oracle-gilts" / "gilts_daily_prices_clickhouse_only.json"
OUT_PATH = ROOT / "json" / "oracle-gilts" / "gilts_prices_gap_from_clickhouse.json"
CACHE_NDJSON = ROOT / "json" / "oracle-gilts" / "gilts_prices_gap_cache.ndjson"
CHUNK = 50_000
...
def last_clickhouse_point() -> tuple[int, str]:
    ...
    # prefer last clickhouse_oracle row if mixed file
...
def load_checkpoint(last_ts: int, last_date: str) -> dict:
    if OUT_PATH.is_file():
        ...
        # Resume by next_block even if daily file was later extended
        if state.get("prices") is not None and state.get("next_block") is not None:
            prices = state.get("prices") or []
            anchor_ts = int(state.get("clickhouse_last_timestamp") or last_ts)
            state["prices"] = prices
            state["count"] = len(prices)
            state["clickhouse_last_timestamp"] = anchor_ts
